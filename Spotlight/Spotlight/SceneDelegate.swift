...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    // MARK: - this method is called when the application has finished launching and it's time to launch the activity requested by the user
    ...
    func scene(_ scene: UIScene, continue userActivity: NSUserActivity) {
        ...
        if userActivity.activityType == CSSearchableItemActionType {
            // unwrap the value of the CSSearchableItemActivityIdentifier - the unique identifier of the indexed item that was tapped
            if let uniqueIdentifier = userActivity.userInfo?[CSSearchableItemActivityIdentifier] as? String {
                if let navigationController = window?.rootViewController as? UINavigationController {
                    if let viewController = navigationController.topViewController as? ViewController {
                        // launch the Safari VC
